...
def blur_image_numpy(src, copy):
    ...
    box_blur = 1./9.
    copy[2:h, 2:w, :c] += src
    copy[2:h, 1:w-1, :c] += src
    copy[2:h, 0:w-2, :c] += src
    copy[1:h-1, 2:w, :c] += src
    copy[1:h-1, 1:w-1, :c] += src
    copy[1:h-1, 0:w-2, :c] += src
    copy[0:h-2, 2:w, :c] += src
    copy[0:h-2, 1:w-1, :c] += src
    copy[0:h-2, 0:w-2, :c] += src
    copy *= box_blur
    return copy[2:h-2, 2:w-2, :c]
