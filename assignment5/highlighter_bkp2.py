...
def color_text(matchobject, code):
    """Recieves a match object and a code to apply to that match.
        Strips the string for old color codes, but keeps the last one to reapply at the end.
        ...
        return:
             (String): A colored version of the match text, to use in substituton
    """
...
def color_print(sourcefile, syntax_dict, theme_dict):
    """Loops through the syntax dict to find matches in the sourcefile. Colors the matches 
        by sending them to color_text together with the color code
        ...
        return:
            sourcefile with color codes inserted
...
def read_syntaxfile(file):
    ...
    """
    syntax_dict = OrderedDict() # Make sure rules are excecuted in order
    try:
