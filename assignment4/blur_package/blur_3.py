...
def main():
    ...
    startTime = time.time()
    dst = blur_image_numba(src, copy, h, w, c)
    endTime = time.time()
...
@jit
def blur_image_numba(src, copy, h, w, c):
        ...
        for j in range(w):
            for k in range(c):
                if i == 0 and j == 0:
                    ...
                    + src[i, j+1, k] + src[i+1, j, k] + src[i+1, j+1, k]) * blur_constant
                elif i == 0:
                    copy[i, j, k] = (src[i, j, k] + src[i, j, k] + src[i+1, j, k]
                    + src[i, j-1, k] + src[i, j+1, k] + src[i, j-1, k]
                    + src[i, j+1, k] + src[i+1, j-1, k] + src[i+1, j+1, k]) * blur_constant
                elif j == 0:
                    copy[i, j, k] = (src[i, j, k] + src[i-1, j, k] + src[i+1, j, k]
                    + src[i, j, k] + src[i, j+1, k] + src[i-1, j, k]
                    + src[i-1, j+1, k] + src[i+1, j, k] + src[i+1, j+1, k]) * blur_constant
                else:
                    copy[i, j, k] = (src[i, j, k] + src[i-1, j, k] + src[i+1, j, k]
